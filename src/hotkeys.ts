...
//          for (var k in _modifier) if (_modifier[k] === key) _hotkeys[k] = true;
//          if (!asterisk) return;
//       }
...
//       if (!(_hotkeys as any).filter.call(this, event)) return;
//       //获取范围 默认为all
//       scope = getScope();

//       //对任何按键做处理
//       if (asterisk) for (var i = 0; i < asterisk.length; i++) {
...
//       if (key in _mods) {
//          _mods[key] = false;
//          for (var k in _modifier) if (_modifier[k] === key) _hotkeys[k] = false;
//       }
//    }
//    //主体hotkeys函数
//    export function _hotkeys(key, scope, method?:any) {
...
//       unbind: unbind
//    };
//    for (var a in _api) _hotkeys[a] = _api[a];

//    export class HotKeys {
//       constructor() {

//       }
//       public bind(keys:string,callback) {
...
//        * @param hotkey 热键
//        */
//       public unbind(keys:string) {
//          (_hotkeys as any).unbind(keys);
//       }
//    }

//    export var hotkeys = new HotKeys();
// }